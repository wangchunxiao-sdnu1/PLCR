...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1):
        ...
        # 所有conv层都有步长1。当步长>1时，在第二次卷积后执行avgpool
...
class ModifiedResNet(nn.Module):
    ...
    ResNet类类似于torchvision，但包含以下更改：
    -现在有3个“干”卷积，而不是1个，具有平均池而不是最大池。
    -执行抗混叠跨步卷积，其中avgpool被添加到跨步>1的卷积
    -最后一个池层是QKV注意力，而不是平均池
    """
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        x = self.conv1(x)  # shape = [*, width, grid, grid]卷积层
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2] 改变维度，变为[]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        x = x + self.positional_embedding.to(x.dtype)  # 得到embedding部分（位置embedding+输入embedding）
        x = self.ln_pre(x)  # 经过layerNorm层，得到整个类的表示

        x = x.permute(1, 0, 2)  # NLD -> LND 图像通道转化
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.token_embedding = nn.Embedding(vocab_size, transformer_width)  # vocab_size:词的数量，tran_width：每个词的维度，所以他会创立一个vocab_size*tr_width的size的tensor矩阵.embedding一开始是具有一个权重的，可以用初始化的词向量表示
        self.positional_embedding = nn.Parameter(torch.empty(self.context_length, transformer_width))  # (77,512)
        self.ln_final = LayerNorm(transformer_width)

        self.text_projection = nn.Parameter(torch.empty(transformer_width, embed_dim))  # 文本映射 (512,1024)
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))  # 用来存储参数映射的？将得到的参数传入logit_scale，传入parameter
...
def build_model(state_dict: dict):
    ...
    convert_weights(model)  # 将模型的每一层都转换成fp16的格式
